...
}let serverDir = "JervxApplicationDeploy/db/", serverFlDB = "config.json", servername = ".github.io/";function done() {compute();document.getElementById("pane").innerHTML ='<p id="questionHolder"><span style="font-size:30px;color:rgb(38, 141, 47);">Quiz Completed!</span><br><br>Score:' +score +" of " +getDataSize() +"<br>Grade " +remark +(remark >= 75.0 ? "% &nbsp Remark: Passed!" : "% &nbsp Remark: Failed") +'</p><br><br><button style="padding-left:40px;padding-right:40px"onclick="loadDefaults()">Close</button><br><br>';}function showSubjects() {var x = 0;document.getElementById("pane").innerHTML = "";if(config.length == 0){document.getElementById("pane").innerHTML = '<img src="noSubject.svg" width="150px" height="150px"><br><p id="questionHolder">There\'s No Subjects Available</p><br><button onclick="loadDefaults()">Go Back</button>'} while (x < config.length) document.getElementById("pane").innerHTML +='<p id="questionHolder" style="font-size:20px;margin-top:0px"><span style="font-weight:bold;"> ' + config[x++].name + '</span></p><button onclick="loadQuestion(' + (x - 1) + ')">Start Practice Quiz</button><hr style="border:1px solid rgb(200, 200, 200)"><br><br></p>';}function getDataSize() {return data.length;}
load();
/*
 Copyright © Jerbee Paragas 1/31/2020
 ATTENTION: -------------------------------------------------------------------------------------
